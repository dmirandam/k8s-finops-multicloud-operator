...
#############################################
# 0. LIMPIEZA IAM (best-effort)
#############################################

echo "🧹 Limpieza IAM previa (si existe). Continuando aunque falle..."
...
aws ec2 create-tags \
  --resources $VPC_ID \
  --tags Key=Name,Value=kfo-multi-eks-vpc
...
#############################################
# 9. EC2 PRIVADA (MISMAS CARACTERÍSTICAS)
#############################################

echo "🔵 Creando Security Group para la EC2 privada..."
...
echo "   ✔️ SG_EC2_PRIVATE = $SG_EC2_PRIVATE"

# Permitir SSH desde el SG del Instance Connect Endpoint
# (Esto es lo recomendable para conectarte a una instancia privada sin abrirla a internet)
aws ec2 authorize-security-group-ingress \
...
echo "🔵 Creando instancia EC2 (Ubuntu 24.04) en subnet privada..."
...
echo "   ✔️ EC2 privada creada con ID: $EC2_PRIVATE_ID"
